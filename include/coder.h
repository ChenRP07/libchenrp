...
namespace coder {
	...
	extern void TurboJpegEncoder(const std::vector<std::vector<uint8_t>>&, FILE* __file_pointer, const int);
	// read a 2D image from a *.jpg file, using a SIMD accelerated turbojpeg library
	extern void TurboJpegDecoder(std::vector<std::vector<uint8_t>>&, FILE* __file_pointer);
