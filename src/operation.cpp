 ...
 * @LastEditTime: 2022-05-17 16:43:48
...
pcl::PointXYZ operation::SubnodeCenter(const pcl::PointXYZ __old_center, const int __position, const float __resolution) {
    ...
    */
    switch (__position) {
    case 0:
        __new_center = pcl::PointXYZ(__old_center.x + __resolution / 2, __old_center.y + __resolution / 2, __old_center.z + __resolution / 2);
        break;
    case 1:
        __new_center = pcl::PointXYZ(__old_center.x + __resolution / 2, __old_center.y + __resolution / 2, __old_center.z - __resolution / 2);
        break;
    ...
    try {
        ...
        if (__merge_node_value == 0x00) {
            ...
            for (size_t i = 0; i < 8; i++) {
                ...
                bool __merge_bit = (__merge_node_value & 0x80) == 0x00 ? true : false;
                bool __node_bit  = (__node_value & 0x80) == 0x00 ? true : false;
