...
/***
 * @description: constructor, default patch number is 50
 * @param {null}
 * @return {null}
 */
SimplePatch::SimplePatch() : Patch() {
    patch_number_ = 50;
}
 ...
 * @return {null}
 */
SimplePatch::SimplePatch(int num) : Patch() {
    patch_number_ = num;
}
...
void SimplePatch::Clustering() {
    ...
    // calculate the centroids from the largest patch_number_ clusters
    pcl::PointCloud<pcl::PointXYZRGB> centroids;
    for (int i = 0; i < this->patch_number_; i++) {
